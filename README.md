...
| lambda\_schedule\_interval | The number of minutes betweeen scheduled runs of the Lambda function to publish egress IP addresses. | `number` | `60` | no |
| lambda\_zip\_filename | The name of the ZIP file containing the Lambda function to publish egress IP addresses.  The file must be located in the root directory of this project. | `string` | `"publish_egress_ip.zip"` | no |
